  ...
  class Deployer
    ...
    def deploy!
      ...
      environments.each do |name, config|
        ...
        shell.run('rancher login', (config['login_options'] if config['login_options']), '-t', "#{config['access_key']}:#{config['secret_key']}", config['server_url'], in: echo_1, only_output_on_error: true)
        # Context switch
